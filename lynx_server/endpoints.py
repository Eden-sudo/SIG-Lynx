...
# Initialize ROS 2 with SingleThreadedExecutor in a background thread
try:
    if not rclpy.ok(): 
        rclpy.init()
    ...
    print(f"[ROS 2] Initialization Error: {e}")
...
async def recibir_y_procesar(file: UploadFile = File(...)):
    # Unique ID to avoid cache collisions
    request_id = str(os.getpid()) + "_" + str(int(asyncio.get_event_loop().time()))
    
    # AUTOMATIC PATH DISCOVERY
    # Uses the script's current directory as base_path
    ...
    print(f"[SERVER] Request received: {file.filename}")
    ...
    try:
        ...
        if not ros_bridge.cli_vision.wait_for_service(timeout_sec=2.0):
            return {"status": "error", "detalle": "Vision Service offline"}
        ...
        while not future_v.done(): 
            await asyncio.sleep(0.05)
        
        resp_v = future_v.result()
        if not resp_v.success:
            return {"status": "error", "detalle": "Visual extraction failed (No paths found)"}

        # 2. OPENCV DRAWING (Neon Filter for Web Preview)
        ...
        print(f"[SERVER] Process completed for {len(rutas)} paths.")
        return {
            "status": "success",
